...
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n'
            + 'FN:Farhan\n'
            + 'ORG:Creator TÔ DE VOLTA BY MAMÃO HEHEHE;\n'
            + 'TEL;type=CELL;type=VOICE;waid=5592988558765:+55 92 98855-8765\n'
            + 'TEL;type=CELL;type=VOICE;waid=559784249346:+55 97 8424-9346\n'
            + 'END:VCARD'

prefix = "_"
name = "~ BOT DO TÔ DE VOLTA BY MAMÃO"
rdaftar = "OBRIGADO POR SE REGISTRAR😁"
rmenu = "Olá amigos do bot do 7 7👋"
limitt = 150
ban = []
userpremium = ["5592988558765,559784249346@s.whatsapp.net"] //ubah nomer kalian
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau amigo i não volte mais@${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
const getRegisteredRandomId = () => {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*⏳ 𝗰𝗮𝗹𝗺𝗮 𝗮í poha tá processando fdp, enquanto isso vai Tomar no cu🤠...*',
				success: '*ꜱᴜceꜱꜱo...*',
				error: {
				...
				only: {
					...
					ownerG: '*𝙨𝙤𝙢𝙚𝙣𝙩e os pika pode usar seu fdp, Membro comum querendo usar comando de ademir rs !*',
		...
		case 'daftar':
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```Lista dos Banidos``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista Users Premium  BOT DO 7 7``` :\n'
