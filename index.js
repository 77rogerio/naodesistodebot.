const {
 ...
 teks = `Halo @ $ {num.split ('@') [0]} \ nSelamat datang di group * $ {mdata.subject} *`
 ...
 teks = `flw man vai fazer falta ñ@ $ {num.split ('@') [0]} já foi tarde 😪👋`
 let buff = await getBuffer (ppimg)
 client.sendMessage (mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"givenJid": [num]}})
 }
 } catch (e) {
 ...
 bagunça = {
 espere: '📌 ​​tá processando irmão📌',
 sucesso: '📌Pronto📌',
 erro: {
 Falha, ocorreu um erro adesivo ao converter a imagem em ❌ ',
  Iv: '❌ Link não é válido
 },
 só: {
 grupo: '❌ Este comando só pode ser usado em grupos!  ❌ ',
 ownerG: '❌ Perintah ini hanya bisa di gunakan oleh grupo de proprietários!  ❌ ',
 ownerB: '❌ Perintah ini hanya bisa di gunakan oleh owner bot!  ❌ ',
 admin: '❌ Perintah ini hanya bisa di gunakan oleh grupo de administração!  ❌ ',
 Badmin: '❌ Perintah ini hanya bisa di gunakan ketika bot menjadi admin!  ❌ '
 }
 }

 const botNumber = client.user.jid
 const ownerNumber = ["92988558765@s.whatsapp.net"] // substitua pelo seu número
 ...
 teks = `* Nama bot *: $ {me.name} \ n * Nomor Bot *: @ $ {me.jid.split ('@') [0]} \ n * Prefixo *: $ {prefix} \ n  * Contato de bloqueio total *: $ {blocks.length} \ n * O bot está ativo em *: $ {kyun (uptime)} `
 buffer = await getBuffer (me.imgUrl)
 client.sendMessage (from, buffer, image, {caption: teks, contextInfo: {associatedJid: [me.jid]}})
 intervalo
 caso 'lista de bloqueio':
